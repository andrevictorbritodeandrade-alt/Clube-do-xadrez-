...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, currentView, setView, logout }) => {
  const menuItems: { id: ViewState; label: string; icon: React.ReactNode }[] = [
    ...
    { 
      id: 'tournaments', 
      label: 'Torneios', 
      icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 21v-8a2 2 0 012-2h14a2 2 0 012 2v8M10 9V5a2 2 0 012-2h0a2 2 0 012 2v4m-6 0h12" /></svg>
    },
    {
      id: 'exercises',
      label: 'Exercícios',
    ...
    {
      id: 'notation',
      label: 'Notação Algébrica',
      icon: <span className="font-bold text-lg w-5 h-5 flex items-center justify-center">Aa</span>
    },
    { 
      id: 'play', 
      label: 'Jogar', 
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-slate-700 h-16">
          <div className="flex items-center space-x-3">
             ...
             <span className="text-xl font-bold tracking-wider">CX MARICÁ</span>
          </div>
        ...
        <div className="p-4 border-t border-slate-700">
          <button 
            onClick={logout}
            className="flex items-center w-full px-4 py-2 text-red-400 hover:bg-slate-800 hover:text-red-300 rounded-lg transition-colors"
          >
          ...
          <p className="text-xs text-slate-500 text-center mt-4">Clube do Xadrez v2.2</p>
        </div>
      </aside>
    </>
  );
